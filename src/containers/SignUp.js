...
const SignUp = () => {
    ...
    const signupController = (e) => {
        ...
        if (dataSignup.firstName.trim() === "") {
            setError(prev => { return { ...prev, firstName: "First Name must be filled" } })
        }
        else {
            setError(prev => { return { ...prev, firstName: "" } })
        }

        if (dataSignup.lastName.trim() === "") {
            setError(prev => { return { ...prev, lastName: "Last Name must be filled" } })
        }
        else {
            setError(prev => { return { ...prev, lastName: "" } })
        }

        if (dataSignup.email.trim() === "") {
            setError(prev => { return { ...prev, email: "Email must be filled" } })
        }
        else {
        ...
        if (dataSignup.password.trim() === "") {
            setError(prev => { return { ...prev, password: "Password is needed" } })
        }
        else {
        ...
        if (dataSignup.password.trim().length < 6) {
            setError(prev => { return { ...prev, password: "Password must has at least 6 characters" } })
        }
        else {
            setError(prev => { return { ...prev, password: "" } })
        }
        ...
        if (dataSignup.confirmPassword.trim() === "") {
            setError(prev => { return { ...prev, confirmPassword: "Please confirm your password" } })
        }
        else if (dataSignup.confirmPassword.trim() !== dataSignup.password.trim()) {
            ...
            <div className="aside">
                ...
                <div className="input-name-wrapper">
                    <div className="input-wrapper">
                        ...
                        <div className="error" id="first-name-error">{error.firstName}</div>
                    </div>
                    <div className="input-wrapper">
                        <input type="text" name="lastName" placeholder="Last name" />
                        <div className="error" id="last-name-error">{error.lastName}</div>
                    </div>
                </div>
                <div className="input-wrapper">
                    <input type="email" name="email" placeholder="Email" />
                    <div className="error" id="email-error">{error.email}</div>
                </div>
                <div className="input-wrapper">
                    <input type="password" name="password" placeholder="Password" />
                    <div className="error" id="password-error">{error.password}</div>
                </div>
                <div className="input-wrapper">
                    <input type="password" name="confirmPassword" placeholder="Confirm your password" />
                    <div className="error" id="confirm-password-error">{error.confirmPassword}</div>
                </div>
