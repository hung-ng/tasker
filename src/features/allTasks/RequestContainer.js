...
const CustomDiv = React.forwardRef(
  ({ children, setJustAdded, setAcceptMember }, ref) => {
    ...
    const acceptUser = async (e) => {
      ...
      try {
        ...
        await db
          ...
          .doc(group_id)
          .update({
            members_id: firebase.firestore.FieldValue.arrayUnion(
              children.email
            ),
    ...
    return (
      <div ref={ref} style={{ width: "20vw" }}>
        <div className="flex requestContent">
          <div className="cursor" title={children.email}>
            {children.name}
          </div>
          <div className="cursor underlined" onClick={acceptUser}>
            Accept
          </div>
        </div>
      </div>
    ...
    <Dropdown>
      ...
      <Dropdown.Menu style={{ overflow: "auto", maxHeight: "50vh" }}>
        {requestList.length === 0 && (
          <div className="noRequest">No pending requests</div>
        )}
        {requestList.length > 0 &&
          requestList.map((req, index) => {
            return (
              <Dropdown.Item
                setAcceptMember={props.setAcceptMember}
