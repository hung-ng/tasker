...
const EditTaskModal = (props) => {
    ...
    const formController = (e) => {
        ...
        if (dataForm.name.trim() !== "" && dataForm.deadline !== "" && dataForm.content.trim() !== "") {
            formModel(dataForm);
        }
    }
        ...
        try {
            ...
            if (props.members_id.length > 0) {
                const res = await db.collection("notifications").add({
                    time: now,
                    content: "Task " + props.task + " was edited",
                    path: "/groups/" + group_id + "/tasks/" + task_id
                })
    ...
    return (
        ...
        >
            ...
            <ModalFooter>
                ...
                <Button disabled={loading} type="submit" form="editTask" variant="primary">Edit</Button>
            </ModalFooter>
        </Modal>
    )
}
