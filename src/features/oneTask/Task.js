...
const Task = () => {
  ...
  const sortComment = (array) => {
    if (currentUser.email !== taskInfo.creator_id) {
      if (visibility === true) {
        return array;
      } else {
        return array.filter(
          (element) =>
            element.user_id === currentUser.email ||
            element.user_id === taskInfo.creator_id
        );
      }
    } else {
      return array;
  ...
  useEffect(() => {
    ...
    async function getTaskInfo() {
      try {
        ...
        const data = res.data();
        if (
          !data.members_id.includes(currentUser.email) &&
          currentUser.email !== data.creator_id &&
          isMounted
        ) {
          setNotExist(true);
        }
  ...
  return (
    <div style={{ width: "100%", margin: "3vh 2vw" }}>
      <EditTaskModal
        members_id={taskInfo.members_id}
        task={taskInfo.name}
        show={showModal}
      ...
      <div className="flex main-header">
        <div className="flex">
          ...
          <div className="flex iconflex">
            <div onClick={editTask} title="Edit" className="icon">
              <FontAwesomeIcon icon={faEdit} size="1x" />
            </div>
            {visibility === true && (
              <div
                onClick={changeVisible}
                title="Comments visible to everyone"
                className="icon"
              >
            ...
            {visibility === false && (
              <div
                onClick={changeVisible}
                title="Comments visible to commentor and creator only"
                className="icon"
              >
                <FontAwesomeIcon icon={faEyeSlash} size="1x" />
              </div>
            )}
            <div onClick={changeTaskStatus} title={markStatus} className="icon">
              <FontAwesomeIcon icon={faCheckCircle} size="1x" />
            </div>
            <div onClick={deleteTask} title="Delete" className="icon">
      ...
      <div
        ...
        dangerouslySetInnerHTML={{ __html: urlify(taskInfo.content) }}
      ></div>
      {taskInfo.attachments && taskInfo.attachments.length > 0 && (
        <div>
          ...
          {taskInfo.attachments.map((attach, index) => {
            return (
              <div key={index}>
                <a target="_blank" rel="noopener noreferrer" href={attach}>
                  <div>{taskInfo.attachmentsName[index]}</div>
                </a>
              </div>
            );
          })}
      ...
      {sortComment(commentInfo).map((comment, index) => {
        return (
          <CommentBar
            status={setNewComment}
            id={comment.comment_id}
            content={urlify(comment.content)}
            time={comment.time}
